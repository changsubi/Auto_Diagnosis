...
class Cephalometric(data.Dataset):
    ...
    def __init__(self, prefix, phase, transform_params=dict(), sigma=10, num_landmark=10, size=[640, 800], use_background_channel=False):
        ...
        n = len(files)
        if phase == 'train':
            self.indexes = files[:702]
        elif phase == 'validate':
            self.indexes = files[702:810]
        elif phase == 'test':
            self.indexes = files
