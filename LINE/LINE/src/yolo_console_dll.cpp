...
int main(int argc, char *argv[])
{
  ...
  for (size_t z = 0; z < xray_type_name.size(); z++)
  {
    ...
    //else if (z == 0)
    {
      ...
      std::cout << "checking x-ray line" << std::endl;
      std::cout << xray_type_name[z] << std::endl;
      std::string cmd_3 = "python3 ./G2unet/demo/main.py -d .runs -r GU2Net_runs -p test -m gln -l u2net -T " + xray_type_name[z] + " -F filelist.csv";
      system(cmd_3.c_str());
      std::string cmd_4 = "python3 ./G2unet/demo/evaluation.py -i .runs/GU2Net_runs/results/test_epoch -T " + xray_type_name[z] + " -F filelist.csv";
      system(cmd_4.c_str());
    }
  }
