...
with open(sys.argv[2], 'r') as read_obj:
    ...
    for row in csv_reader:
        ...
        dicom_list_str = row
"""
for (path, dir, files) in os.walk(".\\eval\\png\\"):
    for filename in files:
        ext = os.path.splitext(filename)[-1]
        if ext == '.json':
            dicom_list_str.append(filename)
...
for kk in range(len(dicom_list_str)-1):
    ...
    with open('seg.csv', 'a', newline='') as fd:
        wr = csv.writer(fd)
        wr.writerow([file_n[0],Overlapping_list,Combined_list,cp_iou_list,"{:.2f}".format(mean_IoU),csv_line_result])
